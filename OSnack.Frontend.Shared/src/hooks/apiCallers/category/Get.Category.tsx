import { AlertObj, AlertTypes, Error } from "../../../components/Texts/Alert";
import { Category } from "../../../_core/apiModels";
import { httpCaller } from "../../../_core/appFunc";
import { API_URL, CommonErrors, ConstMaxNumberOfPerItemsPage } from "../../../_core/constant.Variables";

export const useSearchCategory = async (
   selectedPage = 1,
   maxNumberPerItemsPage = ConstMaxNumberOfPerItemsPage,
   searchValue = "",
   isSortAsce = true,
   sortName = "") => {
   let alert = new AlertObj([], AlertTypes.Error);
   let categoryList: Category[] = [];
   let totalCount = 0;
   try {
      const response = await httpCaller.get(`${API_URL}/Category/Get/Search/${selectedPage}/${maxNumberPerItemsPage}/${searchValue}/${isSortAsce}/${sortName}`);
      switch (response?.status) {
         case 200: // OK
            await response.json().then((data: { list: Category[]; totalCount: number; }) => {
               categoryList = data.list;
               totalCount = data.totalCount;
            });
            break;
         case 417: //Expectation Failed
            await response.json().then((data: Error[]) => {
               alert.List = data;
            });
            break;
         default:
            CommonErrors.BadServerResponseCode.value = `Server Error Code: ${response?.status}`;
            alert.List.push(CommonErrors.BadServerResponseCode);
            break;
      };
      if (response == null)
         alert.List.push(CommonErrors.BadServerConnection);
   } catch (e) {
      alert.List.push(CommonErrors.BadServerResponse);
   }
   return { alert, categoryList, totalCount };
};

export const useGetAllCategory = async () => {
   let alert = new AlertObj([], AlertTypes.Error);
   let categoryList: Category[] = [];
   try {
      const response = await httpCaller.get(`${API_URL}/Category/Get/All`);
      switch (response?.status) {
         case 200: // OK
            await response.json().then((data: Category[]) => {
               categoryList = data;
            });
            break;
         case 417: //Expectation Failed
            await response.json().then((data: Error[]) => {
               alert.List = data;
            });
            break;
         default:
            CommonErrors.BadServerResponseCode.value = `Server Error Code: ${response?.status}`;
            alert.List.push(CommonErrors.BadServerResponseCode);
            break;
      };
      if (response == null)
         alert.List.push(CommonErrors.BadServerConnection);
   } catch (e) {
      alert.List.push(CommonErrors.BadServerResponse);
   }
   return { alert, categoryList };
};