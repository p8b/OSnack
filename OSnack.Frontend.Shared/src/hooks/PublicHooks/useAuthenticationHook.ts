...
import { UserAndIsAuthenticatedAndMaintenanceModeStatusAndIsUserAllowedInMaintenance, ExternalLoginDetails, User, LoginInfo } from "../../_core/apiModels";
export type IReturnUseSilenceAuthentication={ data:UserAndIsAuthenticatedAndMaintenanceModeStatusAndIsUserAllowedInMaintenance , status?: number;};
export const useSilenceAuthentication = (): Promise<IReturnUseSilenceAuthentication> =>{
    ...
    return httpCaller.GET(url_).then(response => {
        ...
        switch(response?.status){
            ...
            case 200: 
                return response?.json().then((data:UserAndIsAuthenticatedAndMaintenanceModeStatusAndIsUserAllowedInMaintenance) => {
